...
<div id="contact_form" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <input type="hidden" name="data[uid]" value="<?php if($this->Session->read('User.uid')) echo $this->Session->read('User.uid'); ?>" />
        <div class="modal-header" style="margin-bottom:10px;padding-bottom:0px;">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
        <h2 id="myModalLabel" class="forh2"><?php echo __("Contact User");?></h2>
        </div>
        ...
        $myuser=ClassRegistry::init('User')->find('first',array('conditions'=>array('uid'=>$this->Session->read('User.uid')))); ?>
        ...
        <div class="controls">
        <?php $tomail=ClassRegistry::init('User')->find('first',array('conditions'=>array('username'=>Configure::read('userpage'))));?>
        <input type="text" id="mail_to" name="data[to]" class="team" placeholder="Email" style="width:425px;padding:2px;" value="<?php echo $tomail['User']['email']?>" readonly="readonly">
        ...
        <div class="control-group " id="mail_msg_div">
        ...
        <span class="help-inline" id="mail_msg_div_error"></span> </div>
        </div>
