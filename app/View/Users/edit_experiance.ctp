<?php 	
	$exp=ClassRegistry::init('Experience')->find('first',array('conditions'=>array('key'=>$this->params['pass'][0])));
...
<div data-backdrop="static" data-keyboard="false" aria-hidden="false" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" class="modal  hide fade in" id="update-resume" style="display:block ;">
	<form class="form-inline" name="update" action="" method="post" enctype="multipart/form-data">
	<input type="hidden" value="<?php echo $this->params['pass'][0];?>" name="data[key]" id="user_id" />
	...
	<h2 id="myModalLabel" style="font-size:15px; padding:0px; margin:0px;"><?php echo __("Professional Experience");?></h2>
        ...
        <?php if($exp['Experience']['logo']!=0) { ?>
			...
			<label class="control-label" for="inputInfo" id="prof" style="width:310px; font-size:14px;font-family:arial;font-weight: bold;"><?php echo __("Job Title");?></label>
				<div id="delme" style=" float:left; width:50px;<?php if($exp['Experience']['logo']==0) { ?>display:none<?php  } ?>">
					<a id="delimg" onclick="return scat();" style="cursor:pointer; color:#foo; <?php if($exp['Experience']['logo']=='') { ?>display:none<?php  } ?>"><img src="<?php echo Router::url('/'); ?>img/delete.png" height="18" width="18" /></a>
				</div>
				...
				<input type="text" placeholder="Job Title" id="job_title" name="data[job_title]" value="<?php echo $exp['Experience']['job_title'];?>" style="padding:2px; margin-right:25px;"> 
					<div class="cp-bl" style="width:150px; float:right;<?php if($exp['Experience']['logo']!=0) { ?>display:none<?php  } ?>" id="mylogo2">
						...
						<div id="ploading" style="display:none;"><?php echo $this->html->image('loading.gif',array());?>Loading....</div>
							<div style="width:150px; float:right; height:50px; display:none; text-align:center; position:relative; bottom:25px;" id="mylogo"></div>
							<span class="help-inline" id="job_title_div_error"></span>
						 </div>
				</div>
